...
def main():
    motor = Motor()
    if motor.get_lower_limit() == 0:
        print("Lower limit already pressed. Down runner will not move.")
        motor.close()
        return

    motor.set_elevator_motor_speed(RUN_SPEED)
    print(
        f"Elevator down runner started at speed {RUN_SPEED} for up to "
        f"{RUN_DURATION:.1f}s."
    )
    ...
    try:
        while True:
            ...
            if motor.get_lower_limit() == 0:
                print("Lower limit pressed. Stopping down runner.")
                break
